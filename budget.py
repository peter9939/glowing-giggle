        ...
        for i in range(start_row, end_row):
            ...
            for j, col in enumerate(df.columns):
                ...
                if isinstance(value, (int, float)):
                    value = f"{value:,.2f}"
                draw.text((j*col_width+5, y+5), str(value), fill=(0,0,0), font=small_font)
...
# --------------------- PDF Generation --------------------- #
def generate_pdf(df_summary, df_cumulative, df_forecast,
                 fig_actual, fig_cum, fig_forecast):

    pdf = PDFReport()
    pdf.set_auto_page_break(auto=True, margin=15)

    # Summary table
    table_images = df_to_image(df_summary, title="Actual vs Budget Summary")
    ...
    pdf.add_page()
    buf = BytesIO()
    fig_actual.write_image(buf, format='png', engine='kaleido', width=900, height=500)
    buf.seek(0)
    pdf.image(buf, w=180)
    ...
    pdf.add_page()
    buf = BytesIO()
    fig_cum.write_image(buf, format='png', engine='kaleido', width=900, height=500)
    buf.seek(0)
    pdf.image(buf, w=180)
    ...
    pdf.add_page()
    buf = BytesIO()
    fig_forecast.write_image(buf, format='png', engine='kaleido', width=900, height=500)
    buf.seek(0)
    pdf.image(buf, w=180)
...
def budget_module():
    ...
    df_filtered['Balance Change'] = df_filtered.apply(
        lambda x: abs(x['Balance Change']) if x['Main Category'].lower() == 'expense'
        else x['Balance Change'], axis=1
    )
    ...
    actual_summary = df_filtered.groupby(['Main Category', 'Subcategory'])['Balance Change'] \
                               .sum().reset_index()
    ...
    cumulative = df_filtered.groupby(['Month','Main Category','Subcategory'])['Balance Change'] \
                            .sum().reset_index()
    cumulative['Cumulative'] = cumulative.groupby(['Main Category','Subcategory'])['Balance Change'] \
                                         .cumsum()

    fig_cum = px.line(cumulative, x='Month', y='Cumulative', color='Subcategory',
                      line_dash='Main Category', markers=True,
                      title='Cumulative Spending')
    st.plotly_chart(fig_cum, use_container_width=True)

    fig_actual = px.bar(actual_summary, x='Subcategory',
    ...
    try:
        forecast_df = df_filtered.groupby(['Month','Main Category','Subcategory'])['Balance Change'] \
                                 .sum().reset_index()
        ...
        fig_forecast = px.line(forecast_display, x='Month', y='Forecast',
                               color='Subcategory',
    ...
    st.download_button(
        ...
        data=generate_pdf(actual_summary, cumulative, forecast_display,
                          fig_actual, fig_cum, fig_forecast),
    ...
    st.download_button(
        ...
        data=excel_buf,
        file_name="budget_forecast_tables.xlsx"
    )
